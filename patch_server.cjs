...
const sep = "// --- Auth + User Routes (Mock/In-memory Implementation) ---";
const parts = text.split(sep);
if (parts.length !== 2) {
  console.error("Separator not found or multiple found.");
  process.exit(1);
}

const top_code = parts[0];

const mongo_logic = `// --- Auth + User Routes (MongoDB Implementation) ---
...
fs.writeFileSync(targetPath, top_code + mongo_logic, 'utf8');
